...
def descargar_resultados_job(job_ref):
    ...
    """
    # No usar job_ref.last_status: la referencia conserva el estado que tenia
    # cuando fue creada y puede quedar desactualizada. Esta llamada consulta el
    # endpoint de Nexus cada vez que se ejecuta la funcion.
    current_status = qnx.jobs.status(job_ref)
    if current_status.status != qnx.jobs.JobStatusEnum.COMPLETED:
        raise RuntimeError(
            f"El job {job_ref.id} aun no esta COMPLETED. "
            f"Estado actual consultado en Nexus: {current_status.status.value}. "
            "Vuelve a ejecutar la celda de consulta mas tarde."
        )

    # Refresca tambien la referencia completa antes de pedir los resultados,
    # evitando reutilizar metadatos cacheados del submit.
    fresh_job_ref = qnx.jobs.get(id=job_ref.id)
    result_refs = list(qnx.jobs.results(fresh_job_ref))
    if not result_refs:
        raise RuntimeError(
            f"El job {job_ref.id} ya figura COMPLETED, pero Nexus aun no publica "
            "resultados descargables. Vuelve a ejecutar la celda de consulta."
        )
...
def ejecutar_local(
    ...
    """
    Ejecuta una funcion Guppy en Selene local.

    ``statevector`` es necesario para kernels ZZ con rotaciones arbitrarias.
    ``stabilizer`` se conserva para programas puramente Clifford.
    """
    emulator = (
        circuito
...
def enviar_job_selene(ref_hugr, n_qubits=2, n_shots=100, nombre=None):
    """
    Envia un HUGR ya subido al simulador vector de estado de Selene/Nexus y devuelve la
